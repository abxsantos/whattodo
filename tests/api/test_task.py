...
@pytest.mark.smoke
def test_task_must_have_description(make_task: Tuple[Task, str]):
    task, example_description = make_task
    assert task.description == example_description


@pytest.mark.smoke
def test_task_must_have_a_created_at():
...
@pytest.mark.smoke
def test_task_status_must_be_false_as_default(make_task: Tuple[Task, str]):
    task, _ = make_task
...
def test_task_description_can_be_updated(make_task: Tuple[Task, str]):
...
@pytest.mark.smoke
def test_task_must_have_str_representation(make_task: Tuple[Task, str]):
    task, description = make_task
    assert str(task) == f"{description} {task.status}"


@pytest.mark.smoke
def test_task_must_have_repr_representation(make_task: Tuple[Task, str]):
    task, description = make_task
